...
Fork the `eth-pkg/eth-nodes` repository and create a branch called `releases/bookworm/amd64/eth-node-nimbus-eth2/24.3.0-1`. Create an empty markdown file in the project outside the `src` directories. Push and wait for GitHub Actions to run.
...
```bash 
sha1sum eth-node-nimbus-eth2_24.3.0-1_amd64.deb 
```
