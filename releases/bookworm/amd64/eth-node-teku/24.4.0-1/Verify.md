...
Fork the `eth-pkg/eth-nodes` repository and create a branch called `verify/bookworm/amd64/eth-node-teku/24.4.0-1`. Create an empty markdown file in the project outside the `src` directories. Push and wait for GitHub Actions to run.
...
```bash 
sha1sum eth-node-teku_5.0.3-1_amd64.deb 
```
