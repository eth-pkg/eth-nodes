...
binary = "/usr/lib/eth-node-nimbus-eth2-testnet/run-nimbus-eth2-service.sh"
user = { name = "eth-node-nimbus-eth2-testnet", group = true, create = { home = false } }
...
# User=eth-node-nimbus-eth2-testnet
...
add_files = [
    "debian/scripts/run-nimbus-eth2-service.sh /usr/lib/eth-node-nimbus-eth2-testnet/", 
    "debian/scripts/run-nimbus-eth2.sh /usr/lib/eth-node-nimbus-eth2-testnet/bin/",
    "debian/scripts/postprocess.sh /usr/lib/eth-node-nimbus-eth2-testnet",
    "debian/tmp/eth-node-nimbus-eth2-testnet.service /lib/systemd/system/",
]
...
[config."nimbus-eth2-testnet.conf".postprocess]
command = ["bash", "/usr/lib/eth-node-nimbus-eth2-testnet/postprocess.sh"]

[config."nimbus-eth2-testnet.conf"]
format = "plain"

[config."nimbus-eth2-testnet.conf".ivars."NIMBUS_ETH2_NON_INTERACTIVE"]
type = "string"
default = "true"
priority = "low"
summary = "Sets non-interactive mode for Nimbus, meaning it will quit on missing configuration."

[config."nimbus-eth2-testnet.conf".ivars."NIMBUS_ETH2_NETWORK"]
type = "string"
default = "$BASE_CONFIG_CUSTOM_NETWORK_TESTNET_DIR"
priority = "low"
summary = "Specifies the Eth2 network to join, typically testnet."

[config."nimbus-eth2-testnet.conf".ivars."NIMBUS_ETH2_DATA_DIR"]
type = "string"
default = "$BASE_CONFIG_DATA_DIR/nimbus-eth2"
priority = "low"
summary = "Specifies the directory where Nimbus will store all blockchain data."

[config."nimbus-eth2-testnet.conf".ivars."NIMBUS_ETH2_WEB3_URL"]
type = "string"
default = "$BASE_CONFIG_ENDPOINT_URL"
priority = "low"
summary = "Specifies one or more execution layer Engine API URLs."

[config."nimbus-eth2-testnet.conf".ivars."NIMBUS_ETH2_JWT_SECRET"]
type = "string"
