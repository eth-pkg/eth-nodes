...
for client in "${CLIENTS[@]}"; do
    if [ -e "$PACKAGE_DIR/eth-node-mainnet-config-$client-1.0.0-1/eth-node-mainnet-$client-1.0.0-1_$ARCH.deb" ] && [ "$REBUILD" == "false" ]; then
        echo "Skipping rebuild as file exists, and no REBUILD flag specified"
    else
        cd eth-node-mainnet-config-$client/1.0.0-1
        pkg-builder verify
        cd ../..
    fi
done
...
for client in "${CLIENTS[@]}"; do
    cp "$PACKAGE_DIR/eth-node-mainnet-config-$client-1.0.0-1/eth-node-mainnet-config-${client}_1.0.0-1_$ARCH.deb" "$SERVE_DIR"
done

# for client in "${CLIENTS[@]}"; do
#     cp "$PACKAGE_DIR/eth-node-mainnet-service-$client-1.0.0-1/eth-node-mainnet-service-${client}_1.0.0-1_$ARCH.deb" "$SERVE_DIR"
# done
