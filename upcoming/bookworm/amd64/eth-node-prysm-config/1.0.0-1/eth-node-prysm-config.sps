...
depends=[]
add_files = [
    "debian/scripts/run-prysm.sh /usr/lib/eth-node-ligthhouse-config/bin/",
    "debian/conf.d /etc/eth-node-prysm-config"
]
add_links = ["/usr/lib/eth-node-ligthhouse-config/bin/run-ligthhouse.sh /usr/bin/run-prysm.sh"]
add_manpages = []
